...
const drawEntity = (() => {
    ...
    function drawPoly(context, centerX, centerY, radius, sides, angle = 0, fill = true) {
        ...
        } else if (sides === 102) { // Pulsar body type
                            for (let i = 0; i < 6; i++) {
        ...
        else if (sides === 106) { // Experimental
                            for (let i = 0; i < 7; i++) {
                                let theta = (i / 5) * 2 * Math.PI,
                                    x = centerX + radius * Math.cos(theta + angle),
                                    y = centerY + radius * Math.sin(theta + angle);
...
const gameDraw = (() => {
    ...
    let tiles = []
    let measureSize = (x, y, colorIndex, {
        index,
        tier = 0
    }) => {
        tiles.push({
            ...
            case 0:
                {
                    ...
                    return {
                        width: us.map(r => r.width).reduce((a, b) => a + b),
                        height: 2 + Math.max(...us.map(r => r.height)),
                    }
                }
        }
    }
    let full = measureSize(0, 0, 0, {
        index: basic.index
    })
 
    let tileSize = Math.min(
        global.screenWidth * 0.9 / full.width,
    ...
    for (let {
        ...
        let ax = global.screenWidth / 2 + (x - full.width / 2) * tileSize + 2
        let ay = global.screenHeight / 2 + (y - full.height / 2) * tileSize + 2
        let size = tileSize - 4
        ...
        let angle = -Math.PI / 4
        let picture = getEntityImageFromMockup(index, gui.color),
...
const gameDrawDead = (() => {
    ...
    return () => {
        ...
        text.taunt.draw(
            'we never know', x, y - 80, 11, color.guiwhite, 'center'
        );
        text.level.draw(
            'level = ' + gui.__s.getLevel() + ' ' + mockups[gui.type].name + '.',
        ...
        text.score.draw(
            'Total score is: ' + formatLargeNumber(Math.round(global.finalScore.get())),
            x - 170, y + 25, 50, color.guiwhite
        );
        text.time.draw(
            'Played for ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.',
