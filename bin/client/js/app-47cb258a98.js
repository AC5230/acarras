...
/***/function (module, exports) {
	...
	var timeForHumans = function timeForHumans(x) {
		...
		weh(seconds, 'second');
		if (y === '') {
			y = 'The FitnessGram Pacer Test is a ... (less than a second, how)';
		}
		return y;
	};
	...
	function getZoneColor(cell, real) {
		switch (cell) {
			...
			case 'rock':
				return color.dgrey;
			case 'nest':
				return real ? color.pink : color.lavender;
			default:
		...
		return {
			...
			remove: function remove(index) {
				if (entries['_' + index] === undefined) {
					console.log('Warning: Asked to remove an undefined leaderboard entry.');
					return -1;
				}
			...
			update: function update(data) {
				if (entries['_' + data.id] === undefined) {
					console.log('Warning: Asked to update an undefined leaderboard entry.');
					return -1;
				}
	...
	// Window setup <3
	global.mobile = /Android|webOS|iOS Devices|BlackBerry/i.test(navigator.userAgent);
	var serverName = 'acarras:glitch:pennsylvania';
					...
					case 'S':
						{
							...
							if (sync.length < 10) {
								...
								global.message = "Game file loaded - " + sync.length + "/10...";
							} else {
					...
					case 'u':
						{
							...
							if (camtime > player.lastUpdate) {
								...
								if (metrics.rendergap <= 0) {
									console.log('Uh-oh. Methinks we goofed again!');
								}
					...
					if (value == null) {
						...
						switch (typeof newValue === "undefined" ? "undefined" : _typeof(newValue)) {
							...
							default:
								...
								throw new Error('Unsupported type for a var!');
						}
					}
					// Update if neeeded
					if (eh) {
	...
	var drawEntity = function () {
		...
		function drawPoly(context, centerX, centerY, radius, sides) {
			...
			if (!sides) {
				...
				for (var _i11 = 0; _i11 < 6; _i11++) {
					var _theta2 = _i11 / 5 * 2 * Math.PI,
					    _x19 = centerX + radius * Math.cos(_theta2 + angle),
					    _y = centerY + radius * Math.sin(_theta2 + angle);
					if (_i11 === 3) {
						_x19 = centerX + 1.6;
						_y = centerY;
					}
					if (_i11 > 3) {
						_x19 = centerX + radius * Math.cos((_i11 === 4 ? 1.18 : 1.58) * Math.PI + angle);
						_y = centerY + radius * Math.sin((_i11 === 4 ? 1.18 : 1.58) * Math.PI + angle);
					}
					context.lineTo(_x19, _y);
				}
			} else if (sides === 103) {
				// Pacman
				for (var _i12 = 0; _i12 < 360; _i12++) {
					var _theta3 = _i12 / 360 * 2 * Math.PI,
					    _x20 = centerX + radius * Math.cos(_theta3 + angle),
					    _y2 = centerY + radius * Math.sin(_theta3 + angle);
					if (_i12 === 135) {
						_x20 = centerX;
						_y2 = centerY;
					} else if (_i12 > 135) {
						_x20 = centerX + radius * Math.cos((_i12 + 90) / 360 * 2 * Math.PI + angle);
						_y2 = centerY + radius * Math.sin((_i12 + 90) / 360 * 2 * Math.PI + angle);
					}
				...
				for (var _i13 = 0; _i13 < 7; _i13++) {
					var _theta4 = _i13 / 5 * 2 * Math.PI,
					    _x21 = centerX + radius * Math.cos(_theta4 + angle),
					    _y3 = centerY + radius * Math.sin(_theta4 + angle);
					if (_i13 === 3) {
					...
					if (_i13 > 4) {
						_theta4 = (_i13 - 2) / 5 * 2 * Math.PI, _x21 = centerX + radius * Math.cos(_theta4 + angle);
						_y3 = centerY + radius * Math.sin(_theta4 + angle);
					}
		...
		return function (x, y, instance, ratio) {
			...
			if (source.guns.length === m.guns.length) {
				...
				for (var _i14 = 0; _i14 < m.guns.length; _i14++) {
					var g = m.guns[_i14],
					    position = positions[_i14] / (g.aspect === 1 ? 2 : 1),
			...
			if (source.turrets.length === m.turrets.length) {
				for (var _i15 = 0; _i15 < m.turrets.length; _i15++) {
					var _t = m.turrets[_i15];
					if (_t.layer === 1) {
						var _ang = _t.direction + _t.angle + rot,
						    _len6 = _t.offset * drawSize;
						drawEntity(xx + _len6 * Math.cos(_ang), yy + _len6 * Math.sin(_ang), _t, ratio, 1, drawSize / ratio / _t.size * _t.sizeFactor, source.turrets[_i15].facing + turretsObeyRot * rot, turretsObeyRot, context, source.turrets[_i15], render);
					}
				}
			} else {
				throw new Error("Mismatch turret number with mockup.");
	...
	var gameDraw = function () {
		...
		var text = {
			...
			upgradeNames: [TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj()],
		...
		return function (ratio) {
			...
			var _loop = function _loop() {
				// Draw Class Tree
				if (!global.showTree) return "break";
				var basic = mockups.find(function (r) {
					return r.name === 'Basic';
				});
				if (!basic) return "break";

				var tiles = [];
				var measureSize = function measureSize(x, y, colorIndex, _ref) {
					var index = _ref.index,
					    _ref$tier = _ref.tier,
					    tier = _ref$tier === undefined ? 0 : _ref$tier;
					...
					var upgrades = mockups[index].upgrades;
					...
					switch (tier) {
						...
						case 2:
							upgrades.forEach(function (u, i) {
								return measureSize(x, y + 2 + i, i, u);
							});
							return {
								width: 1,
						...
						case 0:
							{
								var us = upgrades.map(function (u, i) {
									var measure = measureSize(x, y + 2 * (u.tier - tier), i, u);
									x += measure.width;
									return measure;
								});
								return {
									width: us.map(function (r) {
										return r.width;
									}).reduce(function (a, b) {
										return a + b;
									}),
									height: 2 + Math.max.apply(Math, _toConsumableArray(us.map(function (r) {
										return r.height;
									})))
								};
							}
				...
				var full = measureSize(0, 0, 0, {
					index: basic.index
				});

				var tileSize = Math.min(global.screenWidth * 0.9 / full.width, global.screenHeight * 0.9 / full.height);
				...
				try {
					for (var _iterator = tiles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
						var _step$value = _step.value,
						    _x37 = _step$value.x,
						    _y10 = _step$value.y,
						    _colorIndex = _step$value.colorIndex,
						    index = _step$value.index;

						var ax = global.screenWidth / 2 + (_x37 - full.width / 2) * tileSize + 2;
						var ay = global.screenHeight / 2 + (_y10 - full.height / 2) * tileSize + 2;
						var size = tileSize - 4;

						// Draw box
						ctx.globalAlpha = 0.75;
						ctx.fillStyle = getColor(_colorIndex + 10);
						drawGuiRect(ax, ay, size, size);
						ctx.globalAlpha = 0.15;
						ctx.fillStyle = getColor(_colorIndex);
						...
						var angle = -Math.PI / 4;
						var picture = getEntityImageFromMockup(index, _gui.color),
						    position = mockups[index].position,
						    scale = 0.8 * size / position.axis,
						    _xx = ax + 0.5 * size - scale * position.middle.x * Math.cos(angle),
						    _yy = ay + 0.5 * size - scale * position.middle.x * Math.sin(angle);
						drawEntity(_xx, _yy, picture, 0.5, 1, scale / picture.size * 2, angle, true);

						// Upgrade key
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 2;
						drawGuiRect(ax, ay, size, size, true); // Border
					}
					/*
     let explore = (type, layers) => {
       let mockup = mockups[type]
       if (layers <= 0 || mockup.upgrades.length === 0)
         return { width: 1, children: [], mockup }
       let width = 0
       let children = Array(layers).fill().map(() => [])
       for (let upgrade of mockup.upgrades) {
         let layer = explore(upgrade, layers - 1)
         width += layer.width
         children[0].push(layer)
         for (let i = 0; i < layer.children.length; i++)
           children[i + 1].push(...layer.children[i])
       }
       return { width, children, mockup }
     }
     ctx.fillStyle = '#f00'
     let { width, children } = explore(gui.type, 3)
     let horizontalBlocks = width
     let verticleBlocks = 4
       let tx = global.screenWidth / 2
     let ty = global.screenHeight / 2
     let width = Math.min(global.screenWidth, global.screenHeight) / 2
       for (let tier = 0; tier < children.length; tier++) {
       let x = 0
       for (let { width, mockup } of children[tier]) {
         let start = x + 0.1
         let center = x + width / 2
         let end = x + width - 0.1
         ctx.beginPath()
         ctx.arc(tx, ty, radius * (0.21 + 0.2 * tier),start/totalWidth*Math.PI*2,end/totalWidth*Math.PI*2)
         ctx.arc(tx, ty, radius * (0.39 + 0.2 * tier),end/totalWidth*Math.PI*2,start/totalWidth*Math.PI*2,true)
         ctx.fill()
         x += width
       }
     }
     */
			...
			//https://cdn.discordapp.com/attachments/479009605477728257/527648821879242753/unknown.png
			//https://cdn.discordapp.com/attachments/479009605477728257/527648879504523274/unknown.png

			{
				...
				for (var _i16 = messages.length - 1; _i16 >= 0; _i16--) {
					...
					drawBar(_x31 - msg.len / 2, _x31 + msg.len / 2, _y4 + height / 2, height, color.black);
					// Draw the text
					ctx.globalAlpha = Math.min(1, msg.alpha);
					msg.textobj.draw(_text, _x31, _y4 + height / 2, height - 4, color.guiwhite, 'center', true);
					// Iterate and move
					_y4 += vspacing + height;
					if (msg.status > 1) {
						_y4 -= (vspacing + height) * (1 - Math.sqrt(msg.alpha));
					}
					if (msg.status > 1) {
						msg.status -= 0.05;
						msg.alpha += 0.05;
...

			{
				...
				var save = _len7;
				var _x32 = -spacing - 2 * _len7 + statMenu.get() * (2 * spacing + 2 * _len7);
				var _y5 = global.screenHeight - spacing - _height;
				var ticker = 11;
				...
				_gui.skills.forEach(function drawASkillBar(skill) {
					...
					if (cap) {
						...
						drawBar(_x32 + _height / 2, _x32 - _height / 2 + _len7 * ska(cap), _y5 + _height / 2, _height - 3 + config.graphical.barChunk, color.black);
						drawBar(_x32 + _height / 2, _x32 + _height / 2 + (_len7 - gap) * ska(cap), _y5 + _height / 2, _height - 3, color.grey);
						drawBar(_x32 + _height / 2, _x32 + _height / 2 + (_len7 - gap) * ska(level), _y5 + _height / 2, _height - 3.5, col);
						// Blocked-off area
						if (blocking) {
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x32 + (_len7 - gap) * ska(j), _y5 + 1.5, _x32 + (_len7 - gap) * ska(j), _y5 - 3 + _height);
							}
						}
						// Vertical dividers
						ctx.strokeStyle = color.black;
						...
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x32 + (_len7 - gap) * ska(_j), _y5 + 1.5, _x32 + (_len7 - gap) * ska(_j), _y5 - 3 + _height);
						}
						// Skill name
						_len7 = save * ska(_max);
						var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
						text.skillNames[ticker - 1].draw(name, Math.round(_x32 + _len7 / 2) + 0.5, _y5 + _height / 2, _height - 5, textcolor, 'center', true);
						// Skill key
						text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x32 + _len7 - _height * 0.25) - 1.5, _y5 + _height / 2, _height - 5, textcolor, 'right', true);
						if (textcolor === color.guiwhite) {
							// If it's active
							global.clickables.stat.place(ticker - 1, _x32, _y5, _len7, _height);
						}
						// Skill value
						if (level) {
							text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x32 + _len7 + 4) + 0.5, _y5 + _height / 2, _height - 5, col, 'left', true);
						}
						// Move on 
						_y5 -= _height + _vspacing;
					}
				});
				global.clickables.hover.place(0, 0, _y5, 0.8 * _len7, 0.8 * (global.screenHeight - _y5));
				if (_gui.points !== 0) {
					// Draw skillpoints to spend
					text.skillPoints.draw('x' + _gui.points, Math.round(_x32 + _len7 - 2) + 0.5, Math.round(_y5 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
				}
			}

			{
				...
				var _height2 = 25;
				var _x33 = (global.screenWidth - _len8) / 2;
				var _y6 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				// Handle exp
				// Draw the exp bar
				drawBar(_x33, _x33 + _len8, _y6 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x33, _x33 + _len8, _y6 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x33, _x33 + _len8 * _gui.__s.getProgress(), _y6 + _height2 / 2, _height2 - 3.5, color.gold);
				// Draw the class type
				text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x33 + _len8 / 2, _y6 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', true);
				_height2 = 14;
				_y6 -= _height2 + _vspacing2;
				// Draw the %-of-leader bar
				drawBar(_x33 + _len8 * 0.1, _x33 + _len8 * 0.9, _y6 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x33 + _len8 * 0.1, _x33 + _len8 * 0.9, _y6 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x33 + _len8 * 0.1, _x33 + _len8 * (0.1 + 0.8 * (max ? Math.min(1, _gui.__s.getScore() / max) : 1)), _y6 + _height2 / 2, _height2 - 3.5, color.green);
				// Draw the score
				text.score.draw('Score: ' + handleLargeNumber(_gui.__s.getScore()), _x33 + _len8 / 2, _y6 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', true);
				// Draw the name
				ctx.lineWidth = 4;
				text.name.draw(player.name, Math.round(_x33 + _len8 / 2) + 0.5, Math.round(_y6 - 10 - _vspacing2) + 0.5, 32, color.guiwhite, 'center');
			}

			{
				...
				var _height3 = _len9;
				var _x34 = global.screenWidth - _len9 - spacing;
				var _y7 = global.screenHeight - _height3 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x34 + j++ * _len9 / _W, _y7 + _i17 * _height3 / _H, _len9 / _W, _height3 / _H);
					});
					_i17++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x34, _y7, _len9, _height3);
				minimap.forEach(function (o) {
					if (o[2] === 17) {
						ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.globalAlpha = 0.8;
						drawGuiRect(_x34 + o[0] / global.gameWidth * _len9, _y7 + o[1] / global.gameHeight * _height3, 1, 1);
					} else {
						ctx.strokeStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.lineWidth = 1;
						ctx.globalAlpha = 1;
						drawGuiRect(_x34 + o[0] / global.gameWidth * _len9 - 1, _y7 + o[1] / global.gameWidth * _height3 - 1, 3, 3, true);
						ctx.lineWidth = 3;
					}
				});
				ctx.globalAlpha = 1;
				...
				_x34 + player.x / global.gameWidth * _len9 - 1, _y7 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				ctx.fillStyle = color.black;
				drawGuiRect(_x34, _y7, _len9, _height3, true); // Border

				drawGuiRect(_x34, _y7 - 40, _len9, 30);
				lagGraph(lag.get(), _x34, _y7 - 40, _len9, 30, color.teal);
				gapGraph(metrics.rendergap, _x34, _y7 - 40, _len9, 30, color.pink);
				timingGraph(GRAPHDATA, _x34, _y7 - 40, _len9, 30, color.yellow);
				// Text
				text.debug[5].draw('Prediction: ' + Math.round(GRAPHDATA) + 'ms', _x34 + _len9, _y7 - 50 - 5 * 14, 10, color.guiwhite, 'right');
				text.debug[4].draw('Update Rate: ' + metrics.updatetime + 'Hz', _x34 + _len9, _y7 - 50 - 4 * 14, 10, color.guiwhite, 'right');
				text.debug[3].draw('Latency: ' + metrics.latency + 'ms', _x34 + _len9, _y7 - 50 - 3 * 14, 10, color.guiwhite, 'right');
				text.debug[2].draw('Client FPS: ' + metrics.rendertime, _x34 + _len9, _y7 - 50 - 2 * 14, 10, color.guiwhite, 'right');
				text.debug[1].draw('Server Speed: ' + (100 * _gui.fps).toFixed(2) + '%' + (_gui.fps === 1 ? '' : ' OVERLOADED!'), _x34 + _len9, _y7 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
				text.debug[0].draw(serverName, _x34 + _len9, _y7 - 50, 10, color.guiwhite, 'right');
			}

			{
				...
				var _height4 = 14;
				var _x35 = global.screenWidth - _len10 - spacing;
				var _y8 = spacing + _height4 + 7;
				text.lbtitle.draw('Leaderboard:', Math.round(_x35 + _len10 / 2) + 0.5, Math.round(_y8 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i18 = 0;
				lb.data.forEach(function (entry) {
					drawBar(_x35, _x35 + _len10, _y8 + _height4 / 2, _height4 - 3 + config.graphical.barChunk, color.black);
					drawBar(_x35, _x35 + _len10, _y8 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x35, _x35 + _len10 * shift, _y8 + _height4 / 2, _height4 - 3.5, entry.barcolor);
					// Leadboard name + score 
					text.leaderboard[_i18++].draw(entry.label + ': ' + handleLargeNumber(Math.round(entry.score)), _x35 + _len10 / 2, _y8 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', true);
					// Mini-image
					var scale = _height4 / entry.position.axis,
					    xx = _x35 - 1.5 * _height4 - scale * entry.position.middle.x * 0.707,
					    yy = _y8 + 0.5 * _height4 + scale * entry.position.middle.x * 0.707;
					drawEntity(xx, yy, entry.image, 1 / scale, 1, scale * scale / entry.image.size, -Math.PI / 4, true);
					// Move down
					_y8 += _vspacing3 + _height4;
				});
			}

			{
				...
				if (_gui.upgrades.length > 0) {
					...
					var _height5 = _len11;
					var _x36 = glide * 2 * spacing - spacing;
					var _y9 = spacing;
					var xo = _x36;
					var xxx = 0;
					var yo = _y9;
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y9 > yo) yo = _y9;
						xxx = _x36;
						global.clickables.upgrade.place(_i19++, _x36, _y9, _len11, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x36, _y9, _len11, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						if (colorIndex === 14) colorIndex = 21;
						drawGuiRect(_x36, _y9, _len11, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x36, _y9 + _height5 * 0.6, _len11, _height5 * 0.4);
						ctx.globalAlpha = 1;
						// Find offset location with rotation
						var picture = getEntityImageFromMockup(model, _gui.color),
						    position = mockups[model].position,
						    scale = 0.6 * _len11 / position.axis,
						    xx = _x36 + 0.5 * _len11 - scale * position.middle.x * Math.cos(upgradeSpin),
						    yy = _y9 + 0.5 * _height5 - scale * position.middle.x * Math.sin(upgradeSpin);
						drawEntity(xx, yy, picture, 1, 1, scale / picture.size, upgradeSpin, true);
						// Tank name
						text.upgradeNames[_i19 - 1].draw(picture.name, _x36 + 0.9 * _len11 / 2, _y9 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x36, _y9, _len11, _height5, true); // Border
						if (_ticker++ % 2) {
							_y9 -= _height5 + internalSpacing;
							_x36 += glide * (_len11 + internalSpacing);
						} else {
							_y9 += _height5 + internalSpacing;
						}
					});
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' visitors defeated' : '') : 'rip') + '.';
		};
		...
		return function () {
			...
			text.taunt.draw('we never know', x, y - 80, 11, color.guiwhite, 'center');
			text.level.draw('level = ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name + '.', x - 170, y - 30, 24, color.guiwhite);
			text.score.draw('Total score is: ' + formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.guiwhite);
			text.time.draw('Played for ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.', x - 170, y + 55, 16, color.guiwhite);
